...
async function startServer() {
    ...
    app.post("/api/upload-avatar", async (req, res) => {
        ...
        res.status(501).json({ error: "Not implemented. Use S3 signed upload on client." });
    });
    ...
    if (fs.existsSync(config.tls.cert) && fs.existsSync(config.tls.key)) {
        ...
        logger.warn("TLS cert not found; starting HTTP server (not recommended for production)");
    }
