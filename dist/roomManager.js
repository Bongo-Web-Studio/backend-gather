 ...
 */
export async function joinRoom(roomId, peerId, name, socket) {
 ...
 */
export function leaveRoom(roomId, peerId) {
    const room = rooms.get(roomId);
    if (!room)
        return;
 ...
 */
export async function createTransportForPeer(roomId, peerId) {
 ...
 */
export async function handleProduce(roomId, peerId, producerParams) {
    const room = rooms.get(roomId);
    if (!room)
        throw new Error("room not found");
    const peer = room.get(peerId);
    ...
    for (const [otherId, otherPeer] of room) {
        if (otherId === peerId)
            continue; // skip the producer themself
        try {
